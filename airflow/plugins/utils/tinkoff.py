...
def get_position_by_ticker(ticker: str) -> Optional[tinvest.PortfolioPosition]:
    ...
    filtered_position = list(filter(lambda x: x.ticker.lower() == ticker.lower(), positions))
...
def create_limit_order_by_figi(
    ...
    if op_type not in ('Buy', 'Sell'):
        raise ValueError('Operation type must be Buy or Sell with upper-case first letter')
    client = tinvest.SyncClient(**_get_api_params_from_config())
...
def get_current_balance(currency_type: str) -> float:
    ...
    filtered_currencies = list(filter(lambda x: x.currency.lower() == currency_type.lower(), currencies))
